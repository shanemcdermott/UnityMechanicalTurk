...
public class SimplexNoise : NoiseGenerator
{
    ...
    private short[] p = new short[p_temp.Length];
    ...
    public void init()
    {
        ...
        for (int i = 0; i < 400; i++)
        {
            int from = Random.Range(0, p.Length);
            int to = Random.Range(0, p.Length);
    ...
    private float F2 = 0.5f * (Mathf.Sqrt(3.0f) - 1.0f);
    private float G2 = (3.0f - Mathf.Sqrt(3.0f)) / 6.0f;
    ...
    public override float[,] GenerateHeightMap(int mapWidth, int mapHeight)
    {
        ...
        for (int x = 0; x < mapWidth; x++)
        {
            ...
            {
                float xf = (float)x / mapWidth * scale + offsetX;
                float yf = (float)y / mapHeight * scale + offsetY;
                heights[x, y] = noise(xf, yf);
            }
        }
        return heights;
    }
